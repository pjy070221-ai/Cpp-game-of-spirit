...
public:
    GameplayScene();
    static std::string s_chartPath;   // 谱面路径（由 PackScene 在 push 前设置）
    inline static bool s_randomMode = false;  // 随机轨道模式（由 PackScene 设置）
    inline static bool s_retry = false;        // 重试标志（由 PauseScene / ResultScene 设置）
    inline static bool s_returnToMenu = false;       // 返回主菜单标志（PauseScene 设置 → update 检测）
...
private:
    ...
    std::unique_ptr<sf::Sound> m_tapSound;
