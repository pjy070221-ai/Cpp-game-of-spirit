...
SettingsScene::SettingsScene()
    : m_labels({"Volume", "Note Speed", "Fullscreen", "FPS Limit", "Offset (ms)", "Auto Play", "Difficulty"})
{
...
void SettingsScene::handleEvent(const sf::Event& event) {
    ...
    if (const auto* key = event.getIf<sf::Event::KeyPressed>()) {
        ...
        } else if (m_selection == 6) {  // 难度 Easy/Hard
            v = (v > 0.5f) ? 0.0f : 1.0f;
            s.setDifficulty((int)v);
        }
        s.save();
        refreshDisplay();
        return;
...
void SettingsScene::refreshDisplay() {
    ...
    for (int i = 0; i < (int)m_labels.size(); ++i) {
        ...
        else if (i == 6) val = (v > 0.5f) ? "Hard" : "Easy";
